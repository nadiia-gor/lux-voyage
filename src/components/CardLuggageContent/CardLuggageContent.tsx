import './CardLuggageContent.scss'

interface CardLuggageContent {
    image: string,
    headerText: string,
    priceText: string
  }

  const CardLuggage: React.FC<CardLuggageContent> = ({ image, headerText, priceText }) => {
    return (
      <div className="home--product-card">
        ...
        <div className="image-container"><img src={image} alt={headerText} className="luggage-image" /></div>
        <h3>{headerText}</h3>
        <p>{priceText}</p>
      </div>
    );
